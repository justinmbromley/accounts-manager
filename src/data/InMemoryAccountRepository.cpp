...
// CREATE
std::expected<void, core::CreateAccountError>
InMemoryAccountRepository::add(const core::Account& account) {
...
std::optional<core::Account> InMemoryAccountRepository::find_by_id(const QUuid& id) const {
    const auto it = std::ranges::find_if(
        accounts_, [&id](const core::Account& account) { return account.id() == id; });
...
std::vector<core::AccountSummary>
InMemoryAccountRepository::find_accounts_by_name(const QString& query) const {
...
// UPDATE
std::expected<void, core::UpdateAccountError>
InMemoryAccountRepository::update(const core::Account& account) {
    return {};
}
